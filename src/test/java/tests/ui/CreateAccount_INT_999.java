package tests.ui;

import org.testng.annotations.Test;
import io.qameta.allure.Description;
import io.qameta.allure.Severity;
import io.qameta.allure.SeverityLevel;
import io.qameta.allure.Step;
import pages.ui.CreateAccountPage;
import base.utils.JsonFileReader;
import org.testng.Assert;

public class CreateAccount_INT_999 {

    @Test(description = "INT-999: Create account")
    @Description("Verify that a new account can be created successfully and a confirmation message is displayed.")
    @Severity(SeverityLevel.NORMAL)
    @Step("Steps: Fill create account form → Submit → Verify confirmation message")
    public void testCreateAccount() {
        // Initialize page object
        CreateAccountPage createAccountPage = page.getInstance(CreateAccountPage.class);

        // Load test data from JSON
        String testDataPath = "src/main/resources/Jsons/create-account-INT-999.json";
        org.json.simple.JSONObject testData = JsonFileReader.getTestData(testDataPath, "INT-999");

        // Fill the create account form using data from JSON
        createAccountPage.enterFirstName((String) testData.get("FirstName"));
        createAccountPage.enterMiddleName((String) testData.get("MiddleName"));
        createAccountPage.enterLastName((String) testData.get("LastName"));
        createAccountPage.enterOrganization((String) testData.get("Organization"));
        createAccountPage.enterEmail((String) testData.get("Email"));
        createAccountPage.enterPassword((String) testData.get("Password"));
        createAccountPage.enterConfirmPassword((String) testData.get("ConfirmPassword"));

        createAccountPage.clickCreateAccountButton();

        // Assert confirmation message is displayed
        Assert.assertTrue(createAccountPage.isConfirmationMessageDisplayed(), "Account creation confirmation message was not displayed.");
    }
}
